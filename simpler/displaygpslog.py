    ...
    while True:
		...
		if ((tom != None) or (GPIO.input(38))):
			while (GPIO.input(38)):	
				...
				bl.set_power(True)
				time.sleep(10)
				check="Key Bypass Engaged, " + time.strftime("%d %b %Y %H:%M:%S", time.localtime()) + ",  " + str(gpsd.fix.latitude) + ",  " + str(gpsd.fix.longitude)
				print(check)
				logging.info(check)
			else:
				if (tom != None):
					...
					bl.set_power(True)
					time.sleep(10)
					check="Tom's Here!  :) , " + time.strftime("%d %b %Y %H:%M:%S", time.localtime()) + ",  " + str(gpsd.fix.latitude) + ",  " + str(gpsd.fix.longitude)
					print(check)
					logging.info(check)
		else:
			...
			bl.set_power(False)
			time.sleep(10)
			check="Tom's Gone...  :( , " + time.strftime("%a, %d %b %Y %H:%M:%S", time.localtime()) + ",  " + str(gpsd.fix.latitude) + ",  " +  str(gpsd.fix.longitude)
			print(check)
			logging.info(check)
